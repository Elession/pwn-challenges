...
### Step 3
We need to find a writable location of the binary in memory. That is where we will put the shell payload
...
For this to work, we need to use a pointer gadget so that . Let's use one that has registers that we will be using.
...
## Hints
1. run `file` command on the binary, what is usual?
2. look up linux syscall tables
